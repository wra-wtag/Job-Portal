class ApplicationController < ActionController::Base
  ...
  # Helper method to check if user can access admin area
  def ensure_admin!
    redirect_to root_path unless current_user&.admin?
  end

  # Helper method to check if user can access recruiter area
  def ensure_recruiter!
    redirect_to root_path unless current_user&.recruiter?
  end

  # Helper method to check if user has approved company
